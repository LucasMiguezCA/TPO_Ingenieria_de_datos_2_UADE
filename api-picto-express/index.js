...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://ja646064_db_user:7Lh3NCTTApgDSzA7@ac-5wgfpms-shard-00-00.r6c7til.mongodb.net:27017,ac-5wgfpms-shard-00-01.r6c7til.mongodb.net:27017,ac-5wgfpms-shard-00-02.r6c7til.mongodb.net:27017/picto_mongodb?ssl=true&replicaSet=atlas-9jscho-shard-0&authSource=admin';
...
const LayoutSchema = new mongoose.Schema({
  ...
  nodoPadreId: { type: String, required: true }, // 'raiz' para el inicio, o el ID del nodo padre
  posiciones: [
    {
      pictoId: { type: String, required: true },
      posicion: { type: Number, required: true } // Índice numérico en la grilla (0, 1, 2...)
    }
  ]
}, { timestamps: true });

// Índice compuesto para búsquedas ultra rápidas
LayoutSchema.index({ usuarioId: 1, nodoPadreId: 1 }, { unique: true });

const layout = mongoose.model('Layout', LayoutSchema);


// ANOTACIÓN: Obtener el orden de la grilla para una vista específica
app.get('/layout/:userId/:nodoPadreId', async (req, res) => {
  try {
    const { userId, nodoPadreId } = req.params;
    const layout = await Layout.findOne({ usuarioId: userId, nodoPadreId });
    if (!layout) return res.json({ posiciones: [] });
    res.json(layout);
  } catch (error) {
    res.status(500).json({ mensaje: "Error al obtener layout", error: error.message });
  }
});
// HEALTH
